  ...
  tiffbuffer: array of byte;
        ...
        begin
          ...
          begin
            ...
            if dum < 0 then dum := 0;
            PWord(@tiffbuffer[j * 2])^ := NtoLE(round(dum)); {explicit little-endian, safe on all platforms}
          end;
        end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        PSingle(@tiffbuffer[j * 4])^:= img[0, k, m] / 65535; {write float in range 0..1}
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        if flip_H = true then m := width2 - 1 - j else m := j;
        PSingle(@tiffbuffer[j * 4])^:= img[0, k, m] / 65535; {write floats in range 0..1}
      end;
...
var
  ...
  tiffbuffer: array of byte;
        ...
        begin
          ...
          begin
            if flip_H then m := width2 - 1 - j else m := j;
            base := j * 6;  {each pixel occupies 6 bytes: 3 channels * 2 bytes}
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if flip_H = true then m := width2 - 1 - j else m := j;
        PSingle(@tiffbuffer[j * 12  ])^:= img[0, k, m] / 65535; {write floats in range 0..1}
        PSingle(@tiffbuffer[j * 12+4])^:= img[1, k, m] / 65535; {write floats in range 0..1}
        PSingle(@tiffbuffer[j * 12+8])^:= img[2, k, m] / 65535; {write floats in range 0..1}
      end;
