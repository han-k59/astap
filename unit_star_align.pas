...
procedure find_many_quadsXY(starlist :Tstar_list; out starlistquads :Tstar_list);  {FOR DISPLAY ONLY, Find five times more quads by using closest groups of five stars.}
var
   i,j,k,q,nrstars_min_one,j_index1,j_index2,j_index3,j_index4, nrquads         : integer;
   distance,distance1,distance2,distance3,distance4,x1,x2,x3,x4,xt,y1,y2,y3,y4,yt  : double;
   identical_quad : boolean;
begin
...
//  SetLength(starlistquads,10,nrstars_min_one);{number of quads will be lower}
  SetLength(starlistquads,10,nrstars_min_one*5);{number of quads will be lower}
  ...
  begin
    ...
    distance2:=1E99;{distance second closest star}
    distance3:=1E99;{distance third closest star}
    distance4:=1E99;{distance third closest star}
    ...
    for q:=0 to 4 do//make 5 quads from 5 stars
    begin
      if q=0 then
      begin  //star 0,1,2,3
        x1:=starlist[0,i]; {1e star position}
        ...
        x2:=starlist[0,j_index1]; {2e star position}
        y2:=starlist[1,j_index1];
      ...
      if q=1 then
      begin  //star 1,2,3,4
        x1:=starlist[0,j_index1]; {1e star position}
        y1:=starlist[1,j_index1];

        x2:=starlist[0,j_index2]; {2e star position}
        y2:=starlist[1,j_index2];

        x3:=starlist[0,j_index3];
        y3:=starlist[1,j_index3];

        x4:=starlist[0,j_index4];
        y4:=starlist[1,j_index4];
      end
      else
      if q=2 then
      begin  //star 0,2,3,4
...
begin
  ...
  if nrstars<60 then
  begin
    find_many_quadsXY(starlist, {out}  starlistquads);//Find five times more quads by using closest groups of five stars.
    exit;
  end;
...
begin
  ...
  setlength(star_list_sorted_X,nrstars_min_one+1);
  setlength(star_list_sorted_Y,nrstars_min_one+1);
  for i:=0 to nrstars_min_one do
  begin
  ...
  QuickSort_records_on_X(star_list_sorted_X, 0,nrstars_min_one);
  QuickSort_records_on_Y(star_list_sorted_Y, 0,nrstars_min_one);

  setlength(index_y_stars,nrstars_min_one+1);
  for i:=0 to nrstars_min_one do
...
//  neighbourdistance:=strtoint(mainform1.Edit1.caption);
  ...
  begin
    ...
    begin
      ...
      begin
        j:=i+k;
        if ((j>0) and (j<=nrstars_min_one)) then
        begin
          ...
          begin
            distance:=sqr( star_list_sorted_X[j].x - x1)+ sqr( star_list_sorted_X[j].y-y1);
