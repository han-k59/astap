...
procedure find_best_hyperbola_fit(data:array of TDouble2 ;data_length:integer;out p,a,b, lowest_error : double; out iteration_cycles: integer); {input data[n,1]=position,data[n,2]=hfd, output: bestfocusposition=p, a, b of hyperbola}
...
var
  focus_best   : double; {best focus for command line}
  lowest_error2 : double; {for command line}
...
begin
  ...
  begin
     ...
     if data[i,2] > 0.1 then  {skip near-zero measurements to avoid division blow-up}
       total_error := total_error + abs(hfd_simulation - data[i,2]) / (2+data[i,2]); //linear error, all points contribute equal


     {smart error calculation which limits error for outliers}
    // error:=hfd_simulation - data[i,2] ;{hfd error in simulation}
    // if error < 0 then
    //   total_error:=total_error - error/data[i,2] {if data[i,2] is a large positive outlier then limit error to data[i,2]/data[i,2]=1 maximum}
    //   else
    //   total_error:=total_error + error/hfd_simulation; {if data[i,2] is a large negative outlier then limit error to hfd_simulation/hfd_simulation=1 maximum}
   end;
...
const
  MAX_ITER   = 800;   {maximum number of Nelder-Mead iterations}
...
begin
  ...
  lowest_error2 := lowest_error;{use for command line}
end;
